...
import { Banknote, BriefcaseBusiness, Building2, Factory, FlaskConical, HardHat, Landmark, MonitorSmartphone, Plane, Shield, Sparkles, Truck } from 'lucide-react'
...
export function CompaniesPage({data}:{data:Donation[]}){
  ...
  return <div className="companies-home">
    ...
    <section className="industry-grid wrap" aria-label="Industry sections">{analysis.sections.map(section=>{const Icon=sectionMeta[section.key].icon;return <a href={`#sic-${section.key}`} key={section.key} className="industry-link"><Icon aria-hidden="true"/><div><h2>{section.name}</h2><p>Donated {formatMoney(section.total)}</p></div></a>})}</section>
    <section className="industry-details wrap"><div className="industry-details-head"><p className="eyebrow">Follow the money</p><h2>Companies within each industry</h2><p>Companies are grouped once, using their primary filed SIC code.</p></div>{analysis.sections.map(section=><article className="industry-section" id={`sic-${section.key}`} key={section.key}><header><div><span>SIC section {section.key}</span><h3>{section.name}</h3></div><strong>{formatMoney(section.total)}</strong></header><div className="industry-companies">{section.companies.map((c,i)=><div key={c.number} className="industry-company"><span>{String(i+1).padStart(2,'0')}</span><div><h4>{c.officialName}</h4>{c.names.size>1&&<small>CSV names: {[...c.names].join(' / ')}</small>}<p>{c.codes.length?c.codes.join(' · '):'SIC not filed / unavailable'}</p></div><strong>{formatMoney(c.total)}</strong><a href={`https://find-and-update.company-information.service.gov.uk/company/${c.number}`} target="_blank" rel="noreferrer">Companies House ↗</a></div>)}</div></article>)}</section>
