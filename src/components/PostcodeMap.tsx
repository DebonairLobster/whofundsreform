...
export function PostcodeMap({data,onSelect}:{data:Donation[];onSelect:(donation:Donation)=>void}){
  ...
  const map=useMemo(()=>{
    ...
    const plotted=data.flatMap(d=>{
      ...
      // Pull any displaced coastal point back towards its area centre until it is on UK land.
  ...
  return <div className="map-layout">
    ...
    <section className="panel area-ranking"><h2>Postcode areas</h2><div>{map.items.map(([area,v])=><article key={area}><strong>{area}</strong><span>{v.count} donation{v.count===1?'':'s'}</span><b>{formatMoney(v.total)}</b></article>)}</div><p>{map.unavailable.toLocaleString()} records do not contain a usable postcode area and are excluded from the map.</p></section>
  </div>
}
